...
const servicesOriginal = [
  {
    ...
    code: "STR-X",
    title: "Estratégia",
    desc: "Mapeamento de terreno e planeamento de conquista digital.",
    color: "#fbbf24",
    icon: "◉",
  },
  {
    ...
    desc: "Amplificação de sinal. Atingir o alvo no meio do ruído.",
    color: "#f472b6",
    icon: "◎",
  },
  {
    ...
    desc: "A verdade matemática. Visão estratégica clara.",
    color: "#38bdf8",
    icon: "∰",
  },
  {
    ...
    desc: "Catedrais digitais. Estruturas robustas para suportar impérios.",
    color: "#a78bfa",
    icon: "◈",
  },
  {
    ...
    code: "AUT-0",
    title: "Automação",
    desc: "Eficiência silenciosa e escala infinita.",
    color: "#34d399",
    icon: "⋈",
  },
  {
    ...
    code: "SEC-9",
    title: "Cybersecurity",
    desc: "O escudo invisível contra ameaças hostis.",
    color: "#ef4444",
    icon: "Shield",
  },
];
...
export default function Services() {
  ...
  const scroll = (direction: "left" | "right") => {
    if (scrollContainerRef.current) {
      ...
      const scrollAmount =
        window.innerWidth < 768 ? window.innerWidth * 0.85 : 500;
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.03)_1px,transparent_1px)] bg-size-[60px_60px] mask-[radial-gradient(ellipse_at_center,black_50%,transparent_90%)] pointer-events-none"></div>
      ...
      <div className="px-6 md:px-20 mb-10 flex flex-col md:flex-row justify-between items-end gap-6">
        ...
        <div className="flex gap-4">
          <button
            onClick={() => scroll("left")}
            className="w-16 h-16 rounded-full border border-white/10 bg-white/5 backdrop-blur flex items-center justify-center hover:bg-white/10 hover:border-emerald-500 hover:scale-110 transition-all duration-300 group cursor-pointer"
          >
            <span className="text-2xl text-white group-hover:text-emerald-500 transition-colors">
              &larr;
            </span>
          </button>
          <button
      ...
      >
        {services.map((service, index) => (
          <div
            ...
            className="glass-card relative shrink-0 w-[90vw] md:w-125 h-96 snap-center rounded-4xl p-1 group cursor-pointer transition-all duration-300"
            style={{ transformStyle: "preserve-3d" }}
          >
            {/* Vidro */}
            <div
              className="absolute inset-0 rounded-4xl bg-[#0a0a0a]/80 border border-white/10 backdrop-blur-2xl shadow-2xl overflow-hidden"
            ...
            {/* Borda Neon */}
            <div
              className="holo-border absolute inset-0 rounded-4xl border-2 opacity-0 pointer-events-none transition-opacity duration-300"
              style={{ borderColor: service.color }}
            ></div>

            {/* Conteúdo 3D */}
            <div
              className="holo-content relative h-full flex flex-col justify-between p-10 md:p-12 rounded-[1.8rem]"
            ...
            >
              ...
              <div className="flex justify-between items-start">
                <span className="text-7xl md:text-8xl font-black text-white/5 group-hover:text-white/10 transition-colors select-none">
                  {service.id}
                </span>
                <div className="bg-black/50 border border-white/10 px-3 py-1.5 rounded-full text-[10px] font-mono text-gray-300 uppercase tracking-widest backdrop-blur-md shadow-lg">
                  {service.code}
                </div>
              </div>

              <div className="mt-auto mb-10">
                <div className="text-5xl mb-6 text-white/20 group-hover:text-white transition-colors duration-300 drop-shadow-[0_0_15px_rgba(255,255,255,0.2)]">
                  {service.icon}
                </div>
                <h3
                  className="text-4xl md:text-5xl font-black text-white mb-4 leading-[0.9]"
                  style={{ textShadow: `0 0 30px ${service.color}50` }}
                >
              ...
              <div className="flex items-center gap-3 opacity-30 group-hover:opacity-100 transition-opacity pt-6 border-t border-white/10">
                ...
                ></div>
                <span className="text-[9px] uppercase tracking-[0.2em] text-white whitespace-nowrap">
                  Initialize Protocol
                </span>
