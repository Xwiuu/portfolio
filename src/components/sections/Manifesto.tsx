...
export default function Manifesto() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // 2. Parallax nos Textos (Suavizado para mobile para evitar saltos visuais)
  ...
  return (
    <section 
      id="manifesto" 
      ref={containerRef}
      // Ajuste de padding lateral para mobile
      className="min-h-[150vh] w-full relative z-10 px-8 md:px-20 py-24 md:py-40 overflow-hidden"
    >
      
      {/* LINHA CENTRAL / LATERAL NO MOBILE */}
      ...
      {/* Adicionado pl-8 no mobile para dar espaço à linha lateral */}
      <div className="manifesto-block-left relative w-full md:w-1/2 pl-8 md:pl-0 md:pr-20 mb-32 md:mb-0 text-left md:text-right">
        ...
        <p className="mt-8 text-gray-400 text-sm md:text-base leading-relaxed max-w-md md:ml-auto">
      ...
      {/* --- BLOCO 2: O CAOS (Direita) --- */}
      <div className="manifesto-block-right relative w-full md:w-1/2 pl-8 md:pl-20 md:ml-auto mt-20 md:mt-40">
        ...
        {/* Decorative Code Snippet - Escondido em telas muito pequenas para limpar o visual se necessário, ou mantido como está */}
        <div className="mt-8 p-4 bg-black/40 border border-white/5 font-mono text-[10px] text-gray-500 rounded backdrop-blur-sm overflow-x-hidden">
      ...
      {/* --- BLOCO 3: A CONCLUSÃO (Centro) --- */}
      <div className="relative w-full text-center mt-40 md:mt-60 px-4">
        <h3 className="text-lg md:text-3xl font-light text-white mb-6">
          Não procures um programador.
        </h3>
        <div className="inline-block relative group cursor-none">
          {/* Ajuste de escala do título final para mobile */}
          <h2 className="text-4xl sm:text-6xl md:text-8xl font-black text-transparent bg-clip-text bg-white opacity-20 group-hover:opacity-100 transition-opacity duration-700 select-none leading-none">
            THE ALCHEMIST
          </h2>
          ...
          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-100 mix-blend-overlay">
             <HackerText text="WILLIAM REIS" className="text-4xl sm:text-6xl md:text-8xl font-black text-white" speed={30} />
          </div>
        </div>
      </div>
